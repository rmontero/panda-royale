...
// Largest die each color comes in (rulebook dice distribution) → max legal
// face value. Values above this are clamped to 0 (dropped from scoring) so a
// forged client can't inject an impossible value the UI would reject.
const COLOR_MAX_VALUE = { yellow: 8, purple: 12, blue: 12, red: 8, green: 20, clear: 6, pink: 12 };
...
export function sanitizeDie(d) {
  ...
  let value = Math.max(0, Math.min(99, Math.round(num(d && d.value))));
  // reject out-of-range face values for this color's dice (0 => no contribution)
